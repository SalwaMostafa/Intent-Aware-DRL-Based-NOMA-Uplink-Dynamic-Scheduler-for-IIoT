...
class BaseStation01:
    ...
    def act(self, agent_state):
        ...
        if requesters.size > 0:
            ...
            if len(ue) <= self.n_channels:
                dl_msgs[ue] = self.rng.choice(np.arange(1,self.n_channels+1),size=len(ue),replace=False)
            else:
                dl_msgs[ue[0:math.ceil(len(ue)/2)]] = self.rng.choice(np.arange(1,self.n_channels+1),size=len(ue[0:math.ceil(len(ue)/2)]),replace=False)
                dl_msgs[ue[math.ceil(len(ue)/2):]] = self.rng.choice(np.arange(1,self.n_channels+1),size=len(ue[math.ceil(len(ue)/2):]),replace=False)
        else:
...
class UEAgent01:
    ...
    def act(self, agent_state):
        ...
        dl_msg = np.argmax(agent_state[(((self.hist_msg-1)*self.n_voc_dl)+self.hist_obs):(((self.hist_msg)*self.n_voc_dl)+ self.hist_obs)]) 

        old_action = self.env_action
        # Environment action:
        #print("agent_state",agent_state)
        #print("buffer_status",buffer_status)
        if np.isin(dl_msg, np.arange(1,self.n_channels+1)) and buffer_status > 0:
            action = dl_msg - 1 
        else:
            action = self.n_channels 
        # Communication Action:
        if buffer_status > 0:
            if np.isin(dl_msg, np.arange(0,self.n_channels+2)):
                ul_msg = 1
            else:
                ul_msg = 0
        else:
        ...
        #print("self.env_action",self.env_action)
