...
def register_envs():  
    ...
    gym.envs.register(id='MECSCH-v1',entry_point=MECSCHEnvV2,
         kwargs={'n_ues':50,'n_channels':6,'UE_buffer_capacity':20,'UE_CPU':1e9,'BS_CPU':120e9,'BS_BW':60e6,'hist_msg':1,
                 'hist_comm':1,'hist_obs':1,'hist_act':1,'n_voc_ul':2,'n_voc_dl':8,'arrival_prob':0.7,'max_iters':15,
...
def run_sim_on(env_id="MECSCH-v0",n_episodes=1000,max_ep_len=30,parameter_sharing=False,seed=1024,disable_tqdm=False,
    ...
    feval_reward,feval_success_tasks,feval_channel_success,feval_channel_collision,feval_channel_idle,feval_goodput,feval_droprate,feval_failed = test_agents(agents, env_id, n_episodes=n_feval_episodes, max_ep_len=max_ep_len)
    ...
    return train_rewards,train_success_tasks,train_channel_success,train_channel_collision,train_channel_idle,train_goodput,train_droprate,train_failed,\
           feval_reward,feval_success_tasks,feval_channel_success,feval_channel_collision,feval_channel_idle,feval_goodput,feval_droprate,feval_failed,train_actor_loss,train_critic_loss,train_value 
...
for env,label in zip(envs,labels):
    ...
    eval_every = 50
    n_eval_episodes = 200
    n_feval_episodes = 200
    n_eval = np.int(n_episodes/n_feval_episodes)
    parameter_sharing = False
    disable_tqdm = False

    mappo_params = {"actor_lr":1e-3,"critic_lr":1e-5,"gamma": 0.99,"gae": True,"gae_lmb":0.95,"shuffle": False,"model":"MAPPO"}
...
train_reward = pd.concat([Seed_0_reward,Seed_1_reward],axis=0)
train_reward.to_csv('train_Reward_Data.csv',index=False)

train_success_tasks = pd.concat([Seed_0_Success_Task,Seed_1_Success_Task],axis=0)
train_success_tasks.to_csv('train_success_tasks_Data.csv',index=False)

train_channel_success = pd.concat([Seed_0_channel_success,Seed_1_channel_success],axis=0)
train_channel_success.to_csv('train_channel_success_Data.csv',index=False)

train_channel_collision = pd.concat([Seed_0_channel_collision,Seed_1_channel_collision],axis=0)
train_channel_collision.to_csv('train_channel_collision_Data.csv',index=False)

train_channel_idle = pd.concat([Seed_0_channel_idle,Seed_1_channel_idle],axis=0)
train_channel_idle.to_csv('train_channel_idle_Data.csv',index=False)

train_goodput = pd.concat([Seed_0_goodput,Seed_1_goodput],axis=0)
train_goodput.to_csv('train_goodput_Data.csv',index=False)

train_droprate = pd.concat([Seed_0_droprate,Seed_1_droprate],axis=0)
train_droprate.to_csv('train_droprate_Data.csv',index=False)

train_failed = pd.concat([Seed_0_failed,Seed_1_failed],axis=0)
train_failed.to_csv('train_failed_Data.csv',index=False)

train_actor_loss = pd.concat([Seed_0_actor_loss,Seed_1_actor_loss],axis=0)
train_actor_loss.to_csv('train_actor_loss_Data.csv',index=False)

train_critic_loss = pd.concat([Seed_0_critic_loss,Seed_1_critic_loss],axis=0)
train_critic_loss.to_csv('train_critic_loss_Data.csv',index=False)

train_value = pd.concat([Seed_0_value,Seed_1_value],axis=0)
...
plt.legend(ncol=3, frameon=True, shadow=True,bbox_to_anchor=(1,1.5))
plt.xlabel('Episode')
plt.ylabel("Reward")
...
plt.savefig("actor_loss.jpg",dpi=600,bbox_inches='tight')
plt.show()
...
plt.savefig("critic_loss.jpg",dpi=600,bbox_inches='tight')
plt.show()
...
plt.savefig("action_value.jpg",dpi=600,bbox_inches='tight')
plt.show()